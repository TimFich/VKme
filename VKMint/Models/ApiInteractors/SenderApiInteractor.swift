...
final class SenderApiInteractorImpl: SenderApiInteractor {

    func sendMessage(completion: @escaping ((Int) -> Void), userId: Int?, peerId: Int?, randomId: Int, message: String) {
        if let id = userId {
            VK.API.Messages.send([Parameter.userId: "\(id)",
                                  Parameter.randomId: "\(randomId)",
                                  Parameter.message: "\(message)"])
            .onSuccess({
                let response = try! JSONDecoder().decode(ResponseOfMessage.self, from: $0)
                completion(response.messageId)
            }).send()
        } else {
            VK.API.Messages.send([Parameter.peerId: "\(peerId!)",
                                  Parameter.randomId: "\(randomId)",
                                  Parameter.message: "\(message)"])
            .onSuccess({ result in
